...
def main():
    ...
    print(f"Starting layout rollover across Course Pricing Tabs: '{source_month}' -> '{new_month}'")
    ...
    for sheet_name in COURSE_WORKSHEETS:
        try:
            ...
            # Create duplicate template rows for new month
            new_rows = []
            ...
            print(f"✅ Appended {len(new_rows)} placeholder row(s) for '{new_month}' in '{sheet_name}'.")

        except Exception as e:
            print(f"Error processing worksheet '{sheet_name}': {str(e)}")

    print("🎉 Append month rollover operation completed across all course tabs!")
